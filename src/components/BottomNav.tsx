...
const navItems = [
  ...
  { href: '/subjects', label: '科目', icon: BookIcon },
  { href: '/recall', label: '想起', icon: BrainIcon },
...
export default function BottomNav() {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 glass border-t border-black/[0.06]" />
        ...
        {navItems.map(item => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center pt-2 pb-1 text-[10px] tracking-[0.01em] transition-colors duration-200 ${
                isActive ? 'text-[#5856D6]' : 'text-[#8E8E93]'
              }`}
            >
              <item.icon active={isActive} />
              <span className="mt-[3px] font-medium">{item.label}</span>
            </Link>
          );
        })}
      </div>
...
function HomeIcon({ active }: { active: boolean }) {
  return (
    <svg className="w-[24px] h-[24px]" fill={active ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={active ? 0 : 1.8}>
