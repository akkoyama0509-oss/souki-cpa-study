...
export default function BottomNav() {
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-lg mx-auto flex justify-around">
        {navItems.map(item => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center min-w-[52px] pt-1.5 pb-1 text-[10px] tracking-wide transition-colors ${
                isActive ? 'text-[#5856D6]' : 'text-[#999]'
              }`}
            >
...
function HomeIcon({ active }: { active: boolean }) {
  return (
    <svg className="w-[22px] h-[22px]" fill={active ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={active ? 0 : 1.5}>
